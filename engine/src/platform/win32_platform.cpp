    ...
    i32 height)
{
    ...
    wc.cbClsExtra = 0;
    wc.cbWndExtra = 0;
    wc.hInstance = state->handle_instance;
    wc.hIcon = icon;
    wc.hCursor = LoadCursor(NULL, IDC_ARROW);  // NULL; // Manage the cursor manually
    wc.hbrBackground = NULL;                   // Transparent
    wc.lpszClassName = "ic_window_class";
    ...
    //create handle goes into platform startform
    HWND handle = CreateWindowExA(
        window_ex_style, "ic_window_class", application_name,
        window_style, window_x, window_y, window_width, window_height,
        0, 0, state->handle_instance, 0);

    // goes into plaform startup
    if (handle == 0) {
        MessageBoxA(NULL, "Window creation failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);

        IC_FATAL("Window creation failed!");
        return FALSE;
    } else {
        state->hwnd = handle;
    }
