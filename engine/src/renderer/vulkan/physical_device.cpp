...
namespace ic
{
    bool physical_device::select(
        ...
        IC_CORE_INFO("Selected physical device: {} (Score: {})", m_deviceInfo.properties.deviceName, candidates.rbegin()->first);
        return true;
    }

    bool physical_device::isDeviceSuitable(device_info& deviceInfo, device_requirements& requirements) {
        // check queue families
        queue_family_indices indices = queue_manager::findQueueFamilies(deviceInfo.device, *vulkan_context::get()->getSurface()->get());
        ...
        if (queueFamiliesComplete) {
            ...
            if (!indices.graphicsFamily.has_value()) IC_CORE_WARN("    - Missing graphics queue family");
            if (!indices.presentFamily.has_value()) IC_CORE_WARN("    - Missing present queue family");
            if (!indices.computeFamily.has_value()) IC_CORE_WARN("    - Missing compute queue family");
            if (!indices.transferFamily.has_value()) IC_CORE_WARN("    - Missing transfer queue family");
        }
        ...
        // more feature checks ....
        if (!extensionsSupported) IC_CORE_WARN(" -> Missing required extensions");
        if (!swapChainAdequate) IC_CORE_WARN(" -> Swapchain support inadequate");
        if (!featuresSupported) IC_CORE_WARN(" -> Required features not supported");
    ...
    bool physical_device::checkExtensionSupport(VkPhysicalDevice device) {
        ...
        // all extensions
        IC_CORE_INFO("  Available device extensions ({} total):", extensionCount);
        for (const auto& ext : availableExtensions) {
            IC_CORE_INFO("    - {}", ext.extensionName);
        }

        // required extensions
        IC_CORE_INFO("  Required device extensions ({} total):", m_requirements.requiredExtensions.size());
    ...
    uint32_t physical_device::rateDeviceSuitability(device_info& deviceInfo, device_requirements& requirements) {
        ...
        const auto& indices = queue_manager::findQueueFamilies(deviceInfo.device, *vulkan_context::get()->getSurface()->get());
    ...
    void physical_device::logDeviceInfo() const {
        ...
        const auto& indices = queue_manager::findQueueFamilies(m_deviceInfo.device, *vulkan_context::get()->getSurface()->get());
        
        IC_CORE_INFO("Physical Device Info:");
        IC_CORE_INFO("  Name: {}", properties.deviceName);
        IC_CORE_INFO("  Type: {}", properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU ? "Discrete GPU" : "Other");
        ...
        // IC_CORE_INFO("  Swap Chain Formats: {}", m_deviceInfo.swapChainSupport.formats.size());
        // IC_CORE_INFO("  Present Modes: {}", m_deviceInfo.swapChainSupport.presentModes.size());
    }
} // namespace ic
