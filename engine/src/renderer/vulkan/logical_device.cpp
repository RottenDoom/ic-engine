...
namespace ic
{
    ...
    bool logical_device::create(const physical_device &physicalDevice, VkSurfaceKHR surface) {
        ...
        m_physicalDevice = std::move(const_cast<physical_device&>(physicalDevice));
    ...
    bool logical_device::createLogicalDevice(VkSurfaceKHR surface) {
        ...
        if (deviceFeatures.samplerAnisotropy) IC_CORE_INFO("  - Sampler Anisotropy");
        if (deviceFeatures.fillModeNonSolid) IC_CORE_INFO("  - Fill Mode Non Solid");
        if (deviceFeatures.wideLines) IC_CORE_INFO("  - Wide Lines");
        if (deviceFeatures.geometryShader) IC_CORE_INFO("  - Geometry Shader");
        if (deviceFeatures.tessellationShader) IC_CORE_INFO("  - Tessellation Shader");
