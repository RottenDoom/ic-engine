...
namespace ic
{
        ...
        m_enableValidation(std::move(other.m_enableValidation))
    {
        ...
        if (s_context == &other) s_context = this;
    ...
    bool vulkan_context::initialize(
        GLFWwindow* window,
    ...
    bool vulkan_context::createSwapChain() {
        if (!m_swapchain) m_swapchain = std::make_unique<swapchain>();
        if (!m_swapchain->create(*m_device, *m_surface)) {
    ...
    bool vulkan_context::createSurface(GLFWwindow* window) {
        if (!m_surface) m_surface = std::make_unique<vulkan_surface>();
    ...
    bool vulkan_context::createDevice() {
        if (!m_device) m_device = std::make_unique<logical_device>();
        ...
        if (!m_device->create(physicalDevice, *m_surface->get())) {
